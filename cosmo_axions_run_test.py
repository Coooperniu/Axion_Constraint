            ...
            if line.startswith("#"):
                ...
                try:
                    ...
                    if res[key][0] == '[' and res[key][-1] == ']':
                        ...
                        if res[key][3] != 0.:
                            res[key+'_mean'] = res[key][0]
                            res[key+'_low'] = res[key][1]
                            res[key+'_up'] = res[key][2]
                            res[key+'_sig'] = res[key][3]
                            keys.append(str(key))
                        else:
                            res[key+'_fixed'] = res[key][0]
...
#        los_use_prepared_arrays = False
    ...
    clusters_kwargs = {'omega_Xrays':omegaX,
                      'omega_CMB':omegaCMB,
                      's_IGM':s_IGM,
                      'B_IGM':B_IGM,
...
##########################
    ...
    if params['use_Pantheon'] is True:
        pan_data = load_pantheon(dir_lkl)
    ...
    if params['use_BOSSDR12'] is True:
        boss_data = load_BAO_DR12(dir_lkl)
    ...
    if params['use_BAOlowz'] is True:
        bao_data = load_BAO_lowz(dir_lkl)
    ...
    if params['use_clusters'] is True:
        clusters_data = load_clusters(dir_lkl)
...
##########################
    ...
    mean_x = (0.6847, 0.73, -14, -11, -19.3, 147.78)
    #up_x = (0.75, 0.8, -11, -8, -18, 160)
    #low_x = (0.6, 0.6, -17, -18, -21, 120)

    test_chi2 = lambda x: total_chi2(x,
                           key=keys, param=params,
                           use_SH0ES=params['use_SH0ES'], sh0es_data=shoes_data,
                           use_BAO_DR12=params['use_BOSSDR12'], bao_dr12_data=boss_data,
                           use_BAO_lowz=params['use_BAOlowz'], bao_lowz_data=bao_data,
                           use_Pantheon=params['use_Pantheon'], pan_data=pan_data, pan_kwargs=pan_kwargs,
                           use_TDCOSMO=params['use_TDCOSMO'],
                           use_early=params['use_early'], 
                           use_clusters=params['use_clusters'], clusters_data=clusters_data, err_correct=wanna_correct, fixed_Rvir=fixed_Rvir, clusters_kwargs=clusters_kwargs,
                           verbose=params['verbose'])

    print("Test Chi2:", test_chi2(mean_x))
    #print("Low Chi2:", test_chi2(low_x))
    #print("Up Chi2:", test_chi2(up_x))
    
    ln_kwargs = {'key': keys,
                 'param':params,
                 'use_SH0ES':params['use_SH0ES'],
                 'sh0es_data':shoes_data,
                 'use_BAO_DR12':params['use_BOSSDR12'],
                 'bao_dr12_data':boss_data,
                 'use_BAO_lowz':params['use_BAOlowz'],
                 'bao_lowz_data':bao_data,
                 'use_Pantheon':params['use_Pantheon'],
                 'pan_data':pan_data,
    ...
    p0mean = []
    for key in keys:
        p0mean.append(params[key+'_mean'])
    if params['verbose'] > 0:
        print('keys=%s' % keys)
        print('p0mean=%s' % p0mean)
        print('keys_fixed=%s' % keys_fixed)
        for key in keys_fixed:
            print('fixed param=%s' % params[key+'_fixed'])

    # initial one sigma
    p0sigma = []
    for key in keys:
        p0sigma.append(params[key+'_sig'])
    ...
    if flgmulti:
        with Pool() as pool:
            ...
            try:
                result = sampler.run_mcmc(p0, chainslength, store=True, progress=True)
