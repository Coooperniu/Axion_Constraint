...
def chi2_BAO_lowz(pars, data=None):
    ...
    for i, z in enumerate(BAOlowz_meas_z):
        ...
        if BAO_type[i] == 7:
            ratio = rs / dv
        #print("sdgayudgudfgauigfaugu:", BAO_rs_dV[i], BAO_sigma[i])
        chi2 += ((ratio - BAO_rs_dV[i]) / BAO_sigma[i])**2
        #print(((ratio - BAO_rs_dV[i]) / BAO_sigma[i])**2)
    return chi2
...
def chi2_clusters(pars, data=None, err_correct=True, fixed_Rvir=False, **kwargs):
    ...
    for i in range(len(names)):
        ...
        DA_theory = ADD(cls_z[i], h0, OmL) * ADD_mod(ma, ga, cls_z[i], h0, OmL,
                                                     ne0 = cls_ne0[i],
                                                     ...
                                                     r_up = r_up,
                                                     **kwargs)
        delta_DA = cls_DA[i] - DA_theory
        residuals.append(delta_DA)       


    if err_correct:
        cor_factor += -2. * asymm_cls * (residuals/err_cls) + 5. * asymm_cls**2. * (residuals/err_cls) ** 2.
     
    chi2_list = ((residual / err_cls)**2.) * cor_factor
...
def total_chi2(pars,
    ...
    if not is_Out_of_Range(x, key, param):  # to avoid overflow
        chi2 = 0
...
input_path = "/Users/cooper/Axion/orig_2/inputs/neg_sig_example.param"
...
if param['use_clusters'] is True:
    ...
    clusters_data = None
#
hsh = total_chi2((0.6847, 0.73, -14, -11, -19.3, 147.78), keys=var, params=param,
